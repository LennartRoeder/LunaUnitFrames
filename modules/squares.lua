...
LunaUF2:RegisterModule(Squares, "squares", LunaUF2.L["Status tracker"])
...
local function OnAggro(unit)
	for _,frame in pairs(LunaUF2.Units.frameList) do
		if frame.unit and UnitIsUnit(frame.unit,unit) then
			...
			LunaUF2.modules.borders:OnAggro(frame)
		end
	end
end
...
function Squares:OnEnable(frame)
	if( not frame.squares ) then
		...
		for i = 1, 3 do
			...
			frame.squares.buffs[i]:SetBackdrop(LunaUF2.constants.backdrop)
			...
			frame.squares.debuffs[i]:SetBackdrop(LunaUF2.constants.backdrop)
			...
			frame.squares.trackdebuffs[i]:SetBackdrop(LunaUF2.constants.backdrop)
		...
		frame.squares.aggro:SetBackdrop(LunaUF2.constants.backdrop)
	...
	if not LunaUF2:IsEventRegistered("HealComm_Hotupdate") then
		LunaUF2:RegisterEvent("HealComm_Hotupdate", OnTimer)
	end
	if not LunaUF2:IsEventRegistered("Banzai_UnitGainedAggro") then
		LunaUF2:RegisterEvent("Banzai_UnitGainedAggro", OnAggro)
	end
	if not LunaUF2:IsEventRegistered("Banzai_UnitLostAggro") then
		LunaUF2:RegisterEvent("Banzai_UnitLostAggro", OnAggro)
	end
end
...
function Squares:UpdateTimers(frame)
	if not LunaUF2.db.profile.units.raid.squares.hottracker then	return end
...
function Squares:UpdateAggro(frame)
	...
	if frame.squares then
		if aggro and LunaUF2.db.profile.units.raid.squares.aggro then
...
function Squares:UpdateAuras(frame)
	local config = LunaUF2.db.profile.units.raid.squares
	local i = 1
	local num = 1
	...
	for k,v in pairs(config.buffs.names) do
		if v ~= "" then
			local invert = false
			if k == 1 and LunaUF2.db.profile.units.raid.squares.invertfirstbuff then invert = true end
			if k == 2 and LunaUF2.db.profile.units.raid.squares.invertsecondbuff then invert = true end
			if k == 3 and LunaUF2.db.profile.units.raid.squares.invertthirdbuff then invert = true end
