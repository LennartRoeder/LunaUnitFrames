...
local Health = {}
LunaUF2:RegisterModule(Health, "healthBar", LunaUF2.L["Health bar"], true)
...
local function getGradientColor(unit, startColor)
	...
	if( percent == 0 ) then return LunaUF2.db.profile.healthColors.red end
...
local function classColor(unit)
	...
	return class and LunaUF2.db.profile.classColors[class]
end
...
local function updateTimer()
	...
	if frame.isOffline then
		bar.wasOffline = true
		Health:SetBarColor(bar, LunaUF2.db.profile.units[frame.unitGroup].healthBar.invert, LunaUF2.db.profile.healthColors.offline)
	elseif bar.wasOffline then
		bar.wasOffline = nil
		Health:UpdateColor(frame)
	elseif bar.hasPercent then
		local color
		if ( LunaUF2.db.profile.units[frame.unitGroup].healthBar.classGradient and
		     ( UnitIsPlayer(frame.unit) or UnitCreatureFamily(frame.unit) ) ) then
			color = classColor(frame.unit)
		end
		color = getGradientColor(frame.unit, color or LunaUF2.db.profile.healthColors.green)
		Health:SetBarColor(bar, LunaUF2.db.profile.units[frame.unitGroup].healthBar.invert, color)
	end

	-- Update incoming heal
	if LunaUF2.db.profile.units[frame.unitGroup].incheal.enabled and frame.incheal then
		LunaUF2.modules.incheal:FullUpdate(frame)
	end
end

function Health:OnEnable(frame)
	if( not frame.healthBar ) then
		frame.healthBar = LunaUF2.Units:CreateBar(frame)
...
function Health:SetBarColor(bar, invert, color)
	...
	if( not invert ) then
		bar:SetStatusBarColor(r, g, b, LunaUF2.db.profile.bars.alpha)
		if( not bar.background.overrideColor ) then
			bar.background:SetVertexColor(r, g, b, LunaUF2.db.profile.bars.backgroundAlpha)
		end
	else
		bar.background:SetVertexColor(r, g, b, LunaUF2.db.profile.bars.alpha)
		if( not bar.background.overrideColor ) then
			bar:SetStatusBarColor(0, 0, 0, 1 - LunaUF2.db.profile.bars.backgroundAlpha)
		else
			bar:SetStatusBarColor(bar.background.overrideColor.r, bar.background.overrideColor.g, bar.background.overrideColor.b, 1 - LunaUF2.db.profile.bars.backgroundAlpha)
		end
	end
end
...
function Health:UpdateColor(frame)
	...
	local reactionType = LunaUF2.db.profile.units[frame.unitGroup].healthBar.reactionType
	if( not UnitIsConnected(unit) ) then
		frame.healthBar.wasOffline = true
		self:SetBarColor(frame.healthBar, LunaUF2.db.profile.units[frame.unitGroup].healthBar.invert, LunaUF2.db.profile.healthColors.offline)
		return
	elseif( LunaUF2.db.profile.units[frame.unitGroup].healthBar.colorAggro and UnitThreatSituation(frame.unit) == 3 ) then
		self:SetBarColor(frame.healthBar, LunaUF2.db.profile.units[frame.unitGroup].healthBar.invert, LunaUF2.db.profile.healthColors.hostile)
		return
	elseif( not UnitIsTappedByPlayer(unit) and UnitIsTapped(unit) and UnitCanAttack("player", unit) ) then
		color = LunaUF2.db.profile.healthColors.tapped
	elseif( unit == "pet" and reactionType == "happiness" and GetPetHappiness() ) then
		local happiness = GetPetHappiness()
		if( happiness == 3 ) then
			color = LunaUF2.db.profile.healthColors.friendly
		elseif( happiness == 2 ) then
			color = LunaUF2.db.profile.healthColors.neutral
		elseif( happiness == 1 ) then
			color = LunaUF2.db.profile.healthColors.hostile
		end
	elseif( not UnitPlayerOrPetInRaid(unit) and not UnitPlayerOrPetInParty(unit) and ( ( ( reactionType == "player" or reactionType == "both" ) and UnitIsPlayer(unit) and not UnitIsFriend(unit, "player") ) or ( ( reactionType == "npc" or reactionType == "both" )  and not UnitIsPlayer(unit) ) ) ) then
		if( not UnitIsFriend(unit, "player") and UnitPlayerControlled(unit) ) then
			if( UnitCanAttack("player", unit) ) then
				...
				color = LunaUF2.db.profile.healthColors.enemyUnattack
			end
		elseif( UnitReaction(unit, "player") ) then
			...
			elseif( reaction < 4 ) then
				if( UnitIsCivilian(unit) ) then
					color = LunaUF2.db.profile.healthColors.enemyCivilian
				else
					color = LunaUF2.db.profile.healthColors.hostile
				end
			end
		end
	elseif( LunaUF2.db.profile.units[frame.unitGroup].healthBar.colorType == "class" and ( UnitIsPlayer(unit) or UnitCreatureFamily(unit) ) ) then
		color = classColor(frame.unit)
	elseif( LunaUF2.db.profile.units[frame.unitGroup].healthBar.colorType == "static" ) then
		color = LunaUF2.db.profile.healthColors.static
	end

	if not color or LunaUF2.db.profile.units[frame.unitGroup].healthBar.classGradient then
		color = getGradientColor(unit, color or LunaUF2.db.profile.healthColors.green)
		frame.healthBar.hasPercent = true
	end

	self:SetBarColor(frame.healthBar, LunaUF2.db.profile.units[frame.unitGroup].healthBar.invert, color)
end
...
function Health:FullUpdate(frame)
	local tags = LunaUF2.db.profile.units[frame.unitGroup].tags.bartags.healthBar
	if LunaUF2.db.profile.units[frame.unitGroup].healthBar.vertical then
		frame.healthBar:SetOrientation("VERTICAL")
	else
		frame.healthBar:SetOrientation("HORIZONTAL")
	end
	frame.healthBar:SetReverse(LunaUF2.db.profile.units[frame.unitGroup].healthBar.reverse)
	frame.healthBar.hasPercent = LunaUF2.db.profile.units[frame.unitGroup].healthBar.classGradient
	for align,fontstring in pairs(frame.fontstrings["healthBar"]) do
...
function Health:SetBarTexture(frame,texture)
	if frame.healthBar then
		...
		frame.healthBar:SetStretchTexture(LunaUF2.db.profile.stretchtex)
	end
end
