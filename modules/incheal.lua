...
local Incheal = {}
LunaUF2:RegisterModule(Incheal, "incheal", LunaUF2.L["Incheal"])
local HealComm = LunaUF2.HealComm

local function OnHeal(target)
	for _,frame in pairs(LunaUF2.Units.frameList) do
		if frame.incheal and LunaUF2.db.profile.units[frame.unitGroup].incheal.enabled and frame.unit and UnitName(frame.unit) == target and (UnitInRaid(frame.unit) or UnitInParty(frame.unit) or UnitIsUnit("player",frame.unit)) then
...
function Incheal:OnEnable(frame)
	...
	if not LunaUF2:IsEventRegistered("HealComm_Healupdate") then
		LunaUF2:RegisterEvent("HealComm_Healupdate", OnHeal)
	end
end
...
function Incheal:FullUpdate(frame)
	...
	healBar:SetStatusBarColor(LunaUF2.db.profile.healthColors.inc.r, LunaUF2.db.profile.healthColors.inc.g, LunaUF2.db.profile.healthColors.inc.b, 0.75)
	if healvalue == 0 then
		healBar:Hide()
		return
	end
	...
	if LunaUF2.db.profile.units[frame.unitGroup].healthBar.vertical then
		...
		if (healthHeight + incHeight) > (frameHeight * (LunaUF2.db.profile.units[frame.unitGroup].incheal.cap + 1)) then
			incHeight = (frameHeight * (LunaUF2.db.profile.units[frame.unitGroup].incheal.cap + 1)) - healthHeight
		end
		if incHeight == 0 then
			healBar:Hide()
			return
		...
		if LunaUF2.db.profile.units[frame.unitGroup].healthBar.reverse then
		...
		if (healthWidth + incWidth) > (frameWidth * (LunaUF2.db.profile.units[frame.unitGroup].incheal.cap + 1)) then
			incWidth = (frameWidth * (LunaUF2.db.profile.units[frame.unitGroup].incheal.cap + 1)) - healthWidth
		end
		if incWidth == 0 then
			healBar:Hide()
			return
